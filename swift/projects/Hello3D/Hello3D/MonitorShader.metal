...
//
// Based on https://www.shadertoy.com/view/Wt33Wf
// Converted from WebGL with MoltenGL
...
vertex VertexOut monitorShaderVertex(VertexInput in [[ stage_in ]], constant NodeBuffer& scn_node [[buffer(1)]]) {
  VertexOut out;
...
fragment float4 monitorShaderFragment(VertexOut out [[ stage_in ]], constant SCNSceneBuffer& scn_frame [[buffer(0)]]) {
    ...
    tmpvar_20 = (mix (float3(1.0, 0.2, 1.0), float3(1.0, 0.4, 0.1), (
     ...
     + 0.2)) * (clamp (
      ((tmpvar_18 * (tmpvar_18 * (3.0 -
        ...
        sin(((sunUV_16.y + (0.204 * iTime)) * 100.0))
      ) + clamp (
        ((sunUV_16.y * 14.0) + 1.0)
      , -6.0, 6.0)), 0.0, 1.0))
    , 0.0, 1.0) + (
